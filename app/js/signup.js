document.addEventListener('DOMContentLoaded', function () {
  ...
  // ارسال فرم اطلاعات کاربر و نمایش پیامی مبنی بر تایید به مدیر
  document.getElementById('signup-form').addEventListener('submit', function (event) {
    ...
    fetch('/signup', {
      ...
      if (response.ok) {
        // نمایش پیام موفقیت
        showAlert('کاربر عزیز، اطلاعات حساب شما دریافت شد و جهت تایید به مدیر ارسال گردید.', 'alert-success');
        setTimeout(() => {
          window.location.href = '/signup_success'; // یا صفحه جدید برای نمایش پیام
        }, 3000);
      } else {
        showAlert('خطا در ثبت اطلاعات. لطفا مجددا تلاش کنید.', 'alert-danger');
      }
    }).catch(error => {
      showAlert('خطا در ارسال داده‌ها. لطفا دوباره تلاش کنید.', 'alert-danger');
    });
  });
